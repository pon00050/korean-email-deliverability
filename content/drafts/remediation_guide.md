...
## 6. 네이버/카카오 개별 발신자 등록 (화이트도메인 대체)

### KISA 화이트도메인은 2024년 6월 28일 서비스 종료됐습니다
KISA가 운영하던 중앙화된 화이트리스트는 더 이상 존재하지 않습니다.
네이버와 카카오가 자체 발신자 관리 체계로 전환하면서 폐지됐습니다.

### 현재 대체 경로 — 수신사별 개별 접근

**네이버 메일 (2024년 7월 19일 이후 강화된 요건):**

네이버는 대량 발신자에게 아래 4가지를 의무화했습니다:
1. SPF 설정 ✅
2. DKIM 설정 ✅
3. DMARC 설정 (p=none 이상) ✅
4. PTR(역방향 DNS) 설정 ✅

위 4가지가 갖춰진 상태에서 차단 또는 스팸 분류 문제가 발생하면:
- `help.naver.com` → "스팸 차단 해제 요청" 폼 제출
- 제출 시 필요 정보: 발송 도메인, 발신 IP, 수신자 주소, 발송 시각

**카카오 메일 (Daum/Hanmail):**
- 별도 공개 신청 경로 없음
- 차단 발생 시 카카오 기업 고객센터를 통한 개별 문의
- 대량 발송 시 카카오 공식 파트너 ESP(NHN Cloud, Stibee 등) 경유 권장

### 실질적인 의미
화이트도메인이 없어진 지금, **SPF + DKIM + DMARC + PTR을 올바르게 설정하는 것 자체가 유일한 사전 예방책**입니다. 이 4가지가 갖춰지면 네이버 필터 통과율이 크게 향상됩니다.
